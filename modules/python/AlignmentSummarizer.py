...
class AlignmentSummarizer:
    ...
    def candidates_to_labels(self, candidate_map):
        ...
        for pos in range(self.region_start_position, self.region_end_position):
            if pos in candidate_map:
                candidate_map[pos].print()
            else:
                print("HERE", pos, ref_seq[pos-self.region_start_position])

    def create_summary(self, regions, reads, candidate_map, train_mode):
        all_images = []
        all_labels = []
        all_positions = []
        if train_mode:
            self.candidates_to_labels(candidate_map)
            exit()
            for region in regions:
                region_start, region_end = tuple(region)
                ...
                summary_generator = FRIDAY.SummaryGenerator(ref_seq,
                                                            self.chromosome_name,
                                                            region_start,
                                                            region_end)

                summary_generator.generate_train_summary(reads,
                                                         region_start,
                                                         region_end,
                                                         train_mode)
                exit()
